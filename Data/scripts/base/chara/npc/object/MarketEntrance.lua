...
function onEventStarted(player, npc, triggerName)	
    ...
    -- The three 1.x company offices share the same client-side entrance object.
    -- Inside an office, select the city from the zone rather than the shared
    -- actor class so Hall of Flames exits to Ul'dah (and likewise for the other
    -- two headquarters).
    if (currZone == 232) then
        npcCity = 1;
    elseif (currZone == 234) then
        npcCity = 2;
    elseif (currZone == 233) then
        npcCity = 3;
    end
