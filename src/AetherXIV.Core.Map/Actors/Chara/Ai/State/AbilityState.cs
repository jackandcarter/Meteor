        ...
        public override void OnStart()
        {
            ...
            else
            {
                if (!skill.IsInstantCast())
                {
                    ...
                    owner.GetSubState().chantId = BattleCastPresentationPolicy.GetChantId(
                        owner is Player,
                        skill.castType);
                    DevDiagnostics.Trace(
                        "battle.cast.presentation",
                        "kind", "ability",
                        "phase", "start",
                        "actor", String.Format("0x{0:X}", owner.actorId),
                        "commandId", skill.id,
                        "castType", skill.castType,
                        "chantId", owner.GetSubState().chantId,
                        "waste", owner.GetSubState().waste,
